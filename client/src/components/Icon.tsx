...
// Lucide-Style Stroke-Icons (24x24). Gefüllte Icons sind in FILLED gelistet.
const FILLED = new Set<IconName>(['star', 'play', 'pause', 'stop', 'zap']);
...
const PATHS: Record<IconName, ReactNode> = {
  motorcycle: (
    <>
      ...
      <path d="M5.5 17.5 8.5 11.5h5l4.5 6" />
      <path d="M13.5 11.5 15.5 8h3" />
      <path d="M8.5 11.5 7 8.5H4.5" />
      <circle cx="18.5" cy="8" r="0.9" fill="currentColor" stroke="none" />
    </>
  ),
  camera: (
    <>
...
/** Wiederholt ein Stern-Icon `count`-mal (ersetzt ⭐-Emojis in Listen). */
export function Stars({ count, size = 14 }: { count: number; size?: number }) {
  if (count <= 0) return null;
  return (
    <span className="inline-flex items-center gap-0.5 align-middle text-(--color-warning)">
